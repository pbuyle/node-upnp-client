 ...
 */
ControlPoint.prototype.loadDeviceDescription = function(location, callback) {
...
function searchGateway(timeout, callback) {
  ...
  cp.on('DeviceFound', function(headers) {
    ...
    if (seenDescription[l.href]) return;

    // Retrieve device/service description
    seenDescription[l.href] = l;
    cp.loadDeviceDescription(l, function(err, deviceDescription){
      if (err) {
        callback(err);
        return
      }
      var serviceIdx = deviceDescription.services.length;
      while(serviceIdx--) {
        var service = deviceDescription.services[serviceIdx];
